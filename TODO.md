[NEXT]
UI: move Ground Pivot button to the row above
...
[QUEUE]
UI: EDGE FIXER > Options subrollout toggles slot height in the wrong direction; fix expand/collapse slot-height behavior.
SLICE SELECTED WALLS > RMB: slice selected in both modes; run the currently active radio-button mode first, then run the other mode.
SLICE SELECTED WALLS > Progress: fix progress bar to show actual task completion percentage and task count as n/x.
GLOBAL GEOMETRY: after relevant mesh operations, always check for redundant vertices and remove them automatically; no redundant vertices should be left behind.
HELPERS > Print Objs > LMB: clear Listener and print formatted diagnostic info for selected objects, including object name/type, layer, transform, topology counts, selected sub-objects, vertex positions, face/vertex normals, material/smoothing data, bounding box, and other geometry/debug data needed for troubleshooting.
GET OUTER WALLS > Layer > LMB: after creating the new layer, assign the currently selected objects to that new layer.
